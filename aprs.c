...
void display_fap_error(char *string, struct state *state, fap_packet_t *fap)
{
        ...
        printf("FAP Error %s: %s: %i: %s\n",
               time2str(fap->timestamp, 0), string, *fap->error_code, errbuf);
...
void display_fap_message(struct state *state, fap_packet_t *fap)
{
        ...
        if(fap->message) {
                printf("msg[%d]: %s ", strlen(fap->message), fap->message);
        }
        if(fap->message_ack) {
...
void dump_ui_msg(struct ui_msg *ui_msg)
{
        ...
        printf("ui message [%d]: ", sizeof(struct ui_msg));
...
void save_message(struct state *state, fap_packet_t *fap)
{
        ...
        if(state->msghist[msgind] != NULL) {
                ...
                if( (state->msghist[msgind] = malloc(sizeof(aprsmsg_t))) == NULL) {
...
/*
 * handle the third party packets
 */
void webdisplay_thirdparty(struct state *state, fap_packet_t *fap)
{
        ...
        if( ((nextchar=strchr(fap->orig_packet, ':')) != NULL)
            && *(nextchar+1) == '}' ) {
                strcpy(fapbuf, (nextchar+2));
        } else {
        ...
        if (fap->type == NULL) {
                strcpy(typebuf, "undefined");
        } else {
                sprintf(typebuf, "%02x", *fap->type);
        }
        printf("%s: successfully parsed an encapsultated packet type = %s\n",
               __FUNCTION__, typebuf );
        ...
        if(encap_fap->message) {
                ...
                snprintf(buf, sizeof(buf), "%s->%s@%s: type: %s %s",
...
void parse_incoming_packet(struct state *state, char *packet, int len, int isax25)
{
        ...
        if(fap->type != NULL) {
                /* DEBUG: temporarily only filter message packets */
                if ( (*fap->type == fapMESSAGE) ||
                     fap->message != NULL) {
                        printf("\n%s: %lu, type: 0x%02x -> %s\n",
                ...
                if(state->debug.console_display_filter & CONSOLE_DISPLAY_DEBUG) {
                        printf("%s: %s FAP unhandled packet type\n%s\n",
                               __FUNCTION__, time2str(fap->timestamp, 0), packet);
                }
        }
